    ...
    cell: ({ row }) => {
      ...
      const handleDelete = async () => {
        if (window.confirm(`Tem certeza que deseja deletar o usuário ${user.email}?`)) {
          ...
          try {
            await axios.delete(`/api/users/${user.id}`, { // Assumindo que teremos um endpoint /api/users
...
export function UsersPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const response = await axios.get('/api/users', { // Assumindo que teremos um endpoint /api/users
          headers: { Authorization: `Bearer ${token}` },
        });
